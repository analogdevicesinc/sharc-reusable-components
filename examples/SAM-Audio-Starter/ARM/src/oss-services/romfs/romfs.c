...
static u8 romfs_open_file( const char* fname, FD* pfd, FSDATA *pfs, u32 *plast, u32 *pnameaddr )
{
  ...
  while( 1 )
  {
    ...
    if (fsname[j] == WOFS_END_MARKER_CHAR) {
      *plast = i;
    ...
    {
      is_deleted = fsname[j] == WOFS_FILE_DELETED;
    ...
    // And read the size
    fsize = fsname[j] + ( fsname[ j + 1 ] << 8 );
    fsize += ( fsname[ j + 2 ] << 16 ) + ( fsname[ j + 3 ] << 24 );
    j += ROMFS_SIZE_LEN;
...
static struct dm_dirent* romfs_readdir_r( struct _reent *r, void *d, void *pdata )
{
  ...
  while( 1 )
  {
    ...
    // Look for end of filesystem
    if (header[0] == WOFS_END_MARKER_CHAR) {
      return NULL;
    }
    ...
    {
      is_deleted = header[j] == WOFS_FILE_DELETED;
      j += WOFS_DEL_FIELD_SIZE;
    }
    else
    {
    ...
    // And read the size
    pent->fsize = header[j] + ( header[ j + 1 ] << 8 );
    pent->fsize += ( header[ j + 2 ] << 16 ) + ( header[ j + 3 ] << 24 );
    pent->ftime = 0;
    pent->flags = 0;
    j += ROMFS_SIZE_LEN;
...
int romfs_fsck(int repair, int *repaired)
{
   ...
   while( 1 )
   {
      ...
      if (header[j] == WOFS_END_MARKER_CHAR) {
         return(FS_OK);
      }
      ...
      // Check the size
      fsize = header[j] + ( header[ j + 1 ] << 8 );
      fsize += ( header[ j + 2 ] << 16 ) + ( header[ j + 3 ] << 24 );
      j += ROMFS_SIZE_LEN;
