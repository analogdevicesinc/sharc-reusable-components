...
void shell_ver( int argc, char **argv )
{
  ...
  printf( SHELL_WELCOMEMSG, STR_VERSION );
}
...
void shell_poll( SHELL_CONTEXT *ctx )
{
  ...
  while( 1 )
  {
    ...
    pcmd = shellh_execute_command( ctx->cmd, 1 );
...
int shell_init( SHELL_CONTEXT *ctx, p_term_out term_out, p_term_in term_in, int blocking )
{
