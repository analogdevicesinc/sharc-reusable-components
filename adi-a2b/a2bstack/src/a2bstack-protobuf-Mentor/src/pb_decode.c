...
bool checkreturn pb_decode_varint(pb_istream_t *stream, pb_varint_t *dest)
{
    uint8_t byte;
    uint8_t bitpos = 0;
    pb_varint_t result = 0;
    ...
    do
    {
        ...
        result |= (pb_varint_t)(byte & 0x7F) << bitpos;
        bitpos = (uint8_t)(bitpos + 7);
    } while (byte & 0x80);
