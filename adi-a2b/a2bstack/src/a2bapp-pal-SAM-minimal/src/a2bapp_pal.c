 ...
 * Defines
 **********************************************************************/
#define TIMER_1MS    (SCLK0 / 1000)

/***********************************************************************
 * Timer code
 **********************************************************************/
static uint32_t a2bPalTick;
static ADI_TMR_HANDLE a2bPalTimer;
static uint8_t a2bPalTimerMemory[ADI_TMR_MEMORY];

static void a2bPaltimerCb(void *pCBParam, uint32_t Event, void *pArg)
{
    a2bPalTick++;
}
 ...
 * TWI/I2C code
 **********************************************************************/
static sTWI *a2bPaltwi0Handle;


/***********************************************************************
 * Audio code
 **********************************************************************/
void a2b_clk_init(void)
{
    /* This enables half sport SPORT0 B as a clock master to
     * output a SYNC and BCLK to the on-board AD2425
     */
    *pREG_SPORT0_DIV_B =
        (((SCLK0 / 3072000) - 1) << BITP_SPORT_DIV_B_CLKDIV) |
        ((64) << BITP_SPORT_DIV_B_FSDIV);

    *pREG_SPORT0_CTL_B =
        ((1) << BITP_SPORT_CTL_B_IFS) |
        ((1) << BITP_SPORT_CTL_B_ICLK) |
        ((1) << BITP_SPORT_CTL_B_DIFS) |
        ((1) << BITP_SPORT_CTL_B_SPENPRI) |
        ((32 - 1) << BITP_SPORT_CTL_B_SLEN);
}
...
a2b_HResult pal_timerInit(A2B_ECB * ecb)
{
    ...
    result = adi_tmr_Open (0, a2bPalTimerMemory, ADI_TMR_MEMORY,
        a2bPaltimerCb, NULL, &a2bPalTimer);
    result = adi_tmr_SetMode(a2bPalTimer, ADI_TMR_MODE_CONTINUOUS_PWMOUT);
    result = adi_tmr_SetIRQMode(a2bPalTimer, ADI_TMR_IRQMODE_WIDTH_DELAY);
    result = adi_tmr_SetPeriod(a2bPalTimer, TIMER_1MS);
    result = adi_tmr_SetWidth(a2bPalTimer, TIMER_1MS / 2);
    result = adi_tmr_SetDelay(a2bPalTimer, 0);
    result = adi_tmr_Enable(a2bPalTimer, true);
...
a2b_HResult pal_timerShutdown(A2B_ECB *ecb)
{
   ...
   result = adi_tmr_Close(a2bPalTimer);
...
a2b_Handle pal_i2cOpen(a2b_I2cAddrFmt  fmt, a2b_I2cBusSpeed speed, A2B_ECB *ecb)
{
    ...
    /* Open up a device handle on TWI0 */
    result = twi_open(TWI0, &a2bPaltwi0Handle);
    if (result != TWI_SIMPLE_SUCCESS) {
        a2bPaltwi0Handle = NULL;
    }

    return (a2b_Handle)a2bPaltwi0Handle;
}
...
a2b_HResult pal_i2cClose(a2b_Handle  hnd)
{
    ...
    /* Close the device */
    if (a2bPaltwi0Handle) {
        result = twi_close(&a2bPaltwi0Handle);
    }
    return A2B_RESULT_SUCCESS;
}
