...
static void sport_dma_irq(uint32_t id, void *usrPtr)
{
    ...
    *sport->pREG_DMA_STAT |= ENUM_DMA_STAT_IRQDONE;
    ...
    if (sport->callBack) {
         ...
         *
         * Always give the application a pointer to the buffer not
         * currently being worked on by the DMA.
         */
        pingPong =
            (*sport->pREG_DMA_ADDRSTART == sport->dmaDescriptors[0].start) ?
            1 : 0;

        /* Get the quiescent data buffer address */
        dataAddr = sport->dmaDescriptors[pingPong].start;
