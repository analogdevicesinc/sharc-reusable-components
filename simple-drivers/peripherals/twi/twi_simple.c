...
TWI_SIMPLE_RESULT twi_write(sTWI *twiHandle, uint8_t address, uint8_t *out, uint16_t outLen)
{
    sTWI *twi = twiHandle;
    TWI_SIMPLE_RESULT result;
    ...
    result = twi_xfer(twi, address, TWI_SIMPLE_WRITE, out, outLen, NULL, 0);

    return(result);
}

TWI_SIMPLE_RESULT twi_read(sTWI *twiHandle, uint8_t address, uint8_t *in, uint16_t inLen)
{
    sTWI *twi = twiHandle;
    TWI_SIMPLE_RESULT result;
    ...
    result = twi_xfer(twi, address, TWI_SIMPLE_READ, NULL, 0, in, inLen);
    ...
    uint8_t *out, uint16_t outLen, uint8_t *in, uint16_t inLen)
{
    ...
    result = twi_xfer(twi, address, TWI_SIMPLE_WRITEREAD, out, outLen, in, inLen);
    ...
    *twi->pREG_TWI_ISTAT = istat;
    ...
    if (istat & TWI_SIMPLE_IRQ_MASK) {
       ...
       if (istat & BITM_TWI_ISTAT_TXSERV) {
             ...
             */
            if (twi->longXfer && (twi->slen < TWI_MAX_DCNT)) {
